...
> **Conventions:** these skills adapt the recipe — no per-feature `api`/`impl` modules (`:core:navigation` holds cross-feature keys) and `Navigator` is not provided by Hilt — `MainActivity` creates it around `rememberNavBackStack` (survives process death) and passes it to the installers as a parameter. See `navigation/SKILL.md` → Conventions.
