package com.iwelogic.presentation.views
...
class CollapseTextView : LinearLayout {
    ...
    fun setDefaultExpandState(status: Boolean) {
        isExpand = status
        findViewById<TextView>(R.id.body).visibility = if (isExpand) VISIBLE else GONE
        findViewById<ImageView>(R.id.icon).setImageResource(if (isExpand) R.drawable.collapse else R.drawable.expand)
    }
